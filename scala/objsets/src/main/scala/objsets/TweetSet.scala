...
abstract class TweetSet {
   ...
   */
   def union(that: TweetSet): TweetSet = {
    filterAcc((p: Tweet) => true, that.filter( (p: Tweet) => true))
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(acc: Tweet): Tweet = {
    if(elem.retweets > acc.retweets) mostRetweeted
    else left.mostRetweetedAcc(right.mostRetweetedAcc(acc))
  }

  def filter(p: Tweet => Boolean): TweetSet = {
    if(p(elem)) right.filterAcc(p, left.filter(p)).incl(elem)
    else right.filterAcc(p, left.filter(p))
  }
